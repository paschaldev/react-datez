...
class ReactDatez extends Component {
    ...
    initialiseMonthCalendar() {
        ...
        for (let i = 1; i < 13; i += 1) {
            months.push(<a href={`#${this.state.currentMonthYear}-months-${i}`} className="rdatez-month" key={`${this.state.currentMonthYear}-months-${i}`} onClick={e => this.clickMonth(e, i)}>{moment(i, 'M').format('MMM')}</a>)
        }
    ...
    initialiseYearCalendar() {
        ...
        for (let i = this.state.visibleYear.format('YYYY'); i > (this.state.visibleYear.format('YYYY') - 12); i -= 1) {
            years.push(<a href={`#years-${i}`} className="rdatez-year" key={`years-${i}`} onClick={e => this.clickYear(e, i)}>{i}</a>)
        }
        return years
    }
    ...
    renderCalendar(offset) {
        ...
        for (let i = 1; i <= daysInMonth; i += 1) {
            ...
            calendar.push(<a href={`#${day.format('DMY')}-${i}`} className={dayClasses} key={`${day.format('DMY')}-${i}`} onClick={e => this.clickDay(e, dayDate)} tabIndex="-1">{i}</a>)
        }
